<template>
  <div>
    <div>
      ...
      <svg id="svgSelectionMap" viewBox="0 0 960 500" width="960" height="500" />
      <button type="button" class="button button-blue" id="toAlpPlotButton">go to alpscarf</button>
    </div>
    <div>
      <div id="menus">
    ...
    aoiSelect() {
      ...
      function dragEnd() {
        if (SquareArray.length < maxAmountOfAOI) {
          ...
          if (
            ...
            var nextBox = {
              DatasetStartX: xScale.invert(
                Math.min(selectionRect.originX, selectionRect.currentX)
              ),
              DatasetStartY: yScale.invert(
                Math.max(selectionRect.originY, selectionRect.currentY)
              ),
              DatasetEndX: xScale.invert(
                Math.max(selectionRect.currentX, selectionRect.originX)
              ),
              DatasetEndY: yScale.invert(
                Math.min(selectionRect.currentY, selectionRect.originY)
              ),
      ...
      function removeElement() {
        ...
        for (let i = 0; i < SquareArray.length; i++) {
          if (
            Math.round(xScale(SquareArray[i].DatasetStartX)) ==
              this.x.baseVal.value &&
            (Math.round(yScale(SquareArray[i].DatasetEndY)) ==
              this.y.baseVal.value ||
              Math.round(yScale(SquareArray[i].DatasetStartY)) ==
                this.y.baseVal.value)
          ) {
            possibleAoiNames.push(SquareArray[i].numberBox);
            data.forEach(function(d) {
              if (
                d.MappedFixationPointX > SquareArray[i].DatasetStartX &&
      ...
      const FilterAOI = RectangleBox => {
        ...
        data.forEach(function(d) {
          if (
            d.MappedFixationPointX > RectangleBox.DatasetStartX &&
      ...
      function rectanglesIntersect(rectOne, rectTwo) {
        var maxOneX = Math.max(
          Math.round(xScale(rectOne.DatasetStartX)),
          Math.round(xScale(rectOne.DatasetEndX))
        );
        var minOneX = Math.min(
          Math.round(xScale(rectOne.DatasetStartX)),
          Math.round(xScale(rectOne.DatasetEndX))
        );
        var maxOneY = Math.max(
          Math.round(yScale(rectOne.DatasetStartY)),
          Math.round(yScale(rectOne.DatasetEndY))
        );
        var minOneY = Math.min(
          Math.round(yScale(rectOne.DatasetStartY)),
          Math.round(yScale(rectOne.DatasetEndY))
        );
        var maxTwoX = Math.max(
          Math.round(xScale(rectTwo.DatasetStartX)),
          Math.round(xScale(rectTwo.DatasetEndX))
        );
        var minTwoX = Math.min(
          Math.round(xScale(rectTwo.DatasetStartX)),
          Math.round(xScale(rectTwo.DatasetEndX))
        );
        var maxTwoY = Math.max(
          Math.round(yScale(rectTwo.DatasetStartY)),
          Math.round(yScale(rectTwo.DatasetEndY))
        );
        var minTwoY = Math.min(
          Math.round(yScale(rectTwo.DatasetStartY)),
          Math.round(yScale(rectTwo.DatasetEndY))
        );
        return (
      ...
      d3.select("#selectMenu").on("change", function() {
      ...
      d3.csv("static/csv/resolution.csv").then(loadedData => {
        resData = loadedData;
      ...
      function visualize(kamal) {
        ...
        loadAndProcessData(kamal).then(loadedData => {
          ...
          alpVizHeight =
            (height - margin.top - margin.bottom) / pNameList.length;
